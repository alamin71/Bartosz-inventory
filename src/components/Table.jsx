...
//       {/* 2- Expected results based on selected packages with sidebar */}
//       <div className="bg-white rounded-lg p-4 flex">
...
//       {/* 3️- Price of selected packages with sidebar */}
//       <div className="bg-white rounded-lg p-4 flex">
...
const TableComponent = ({ packages }) => {
  ...
  // 🧠 Memoized calculateSalesSavings to avoid re-creation on each render
  const calculateSalesSavings = useCallback(
    (packageItem, input) => {
      let savings = 0;
      if (input && input > 0) {
        switch (packageItem.id) {
  ...
  // Package price & savings calculation
  useEffect(() => {
    const totalPackagePrice = packages.reduce((acc, pkg) => acc + pkg.price, 0);
    setTotalPrice(totalPackagePrice * numOfPremises);
  ...
  useEffect(() => {
    ...
    setTotalMonthlySavings(totalMonthly);
  }, [inputValues, numOfPremises, packages, calculateSalesSavings]);
